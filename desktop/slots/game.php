...
$providerCode = $_GET['provider'] ?? 'pragmatic';
    ...
    <div class="container sub-games">
        <div class="">
            ...
            if(isset($_GET['provider'])){
              ...
              $sql_provider = $conn->prepare("SELECT * FROM `tb_provider` WHERE slug = ?");
              $sql_provider->bind_param("s", $provider);
              $sql_provider->execute();
              $result_provider = $sql_provider->get_result();
              $sp = $result_provider->fetch_assoc();
            ...
            $sql_3 = $conn->prepare("SELECT * FROM `game_list` WHERE $where ");
            $sql_3->bind_param("s", $providerCode);
            $sql_3->execute();
            $result_games = $sql_3->get_result();
            while($s3 = $result_games->fetch_assoc()){

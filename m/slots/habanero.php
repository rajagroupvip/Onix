...
<div class="container pt-1 sub-games">
    ...
    <div class="row games no-gutters">
        <?php
                    ...
                    $where = "`provider` = 'HB' AND";
                    $pageLink = 'slot/?';
                    }
                    
                    $sql_3 = mysqli_query($conn,"SELECT * FROM `tb_games` WHERE $where `datatype` = 'RNG' ORDER BY cuid ASC") or die(mysqli_error($conn));
                    while($s3 = mysqli_fetch_array($sql_3)){
                    if(isset($_SESSION['user'])){
                        $externalPlayerId = $_SESSION['user'];
                        $playUrl = $urlweb.'/gameplay2/'.$s3['provider'].'/?gamecode='.$s3['gameid'].'&extplayer='.$externalPlayerId;
                ...
                ?>
        <a class="col-xs-4 col-md-3 game-box text-center" href="javascript:void(0);" onclick="registerPopup({content:'Silahkan Buka Game Yang Lain'})""
            data-title="<?php echo $s3['gamename']; ?>" data-filter="ALL,Video Slots,TOP,Buy Bonus Feature"
            [ngClass]="{'flex-grow-2' : game.FlexGrow =='2'}"
            onclick="showGameLinks(event,    '<?php echo $s3['gamename']; ?>', '<?php echo $urlweb . '/' . $s3['image']; ?>')">
            <div class="content-wrapper">
                <div class="daily-wins-tag"> </div>
                <img width=120 height=120 class="lazy" data-src="<?php echo $urlweb . '/' . $s3['image']; ?>">
            </div>
            <h5 data-title="<?php echo $s3['gamename']; ?>"><?php echo $s3['gamename']; ?></h5>
        </a>
        <?php } ?>
    </div>

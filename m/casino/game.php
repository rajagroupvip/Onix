...
$providerCode = isset($_GET['provider']) ? $_GET['provider'] : '';
...
$query = "SELECT * FROM provider where provider_type='lc' ";
...
if ($result) {
    ...
    while ($row = mysqli_fetch_assoc($result)) {
        echo '<div class="col">';
        echo '<a class="btn-box" href="/m/casino/game/?provider=' . $row['code'] . '" rel="opener">';
...
?>
    ...
    <?php
        ...
        $sql_3 = mysqli_query($conn, "SELECT * FROM `casino_game` WHERE $where ") or die(mysqli_error($conn));

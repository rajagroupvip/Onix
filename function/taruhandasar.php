...
if($saldoAktif < $subtotal){
	header('location:../taruhan.php?pid='.$pid.'&gameid=13&notif=1');
	exit();
}
else {
	for($i=0;$i<$jumlah;$i++){
		...
		$getGame = mysqli_query($conn,"SELECT * FROM `tb_game` WHERE parent = 13 AND title = '$angka'") or die(mysqli_error());
		...
		if($nominal != ''){
			...
			$insert_taruhan = mysqli_query($conn,"INSERT INTO `tb_taruhan` (`userID`, `pid`, `gameid`, `code`, `periode`, `created_date`, `tebak`, `posisi`, `nominal`, `menang`, `diskon`, `bayar`, `jumlah`, `keterangan`, `status`) VALUES ('$useridnya', '$pid', 13, '$kodenya', '$periode', '$created_date', '$angka', '', '$nominal', '$menang', '$diskon', '$totalBayar', '$jumlahMenang', '', 0)") or die(mysqli_error());

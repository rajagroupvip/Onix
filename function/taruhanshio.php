...
if($saldoAktif < $subtotal){
	header('location:../taruhan.php?pid='.$pid.'&gameid=22&notif=1');
	exit();
}
else {
	for($i=0;$i<$jumlah;$i++){
		$tebak = $_POST['angka'][$i];
		$explode = explode('_',$tebak);
		$angka = $explode[0];
		$posisi = $explode[1];
		$nominal = $_POST['nominal'][$i];
		$getGame = mysqli_query($conn,"SELECT * FROM `tb_game` WHERE cuid = 22") or die(mysqli_error());
		...
		if($nominal != ''){
			...
			$insert_taruhan = mysqli_query($conn,"INSERT INTO `tb_taruhan` (`userID`, `pid`, `gameid`, `code`, `periode`, `created_date`, `tebak`, `posisi`, `nominal`, `menang`, `diskon`, `bayar`, `jumlah`, `keterangan`, `status`) VALUES ('$useridnya', '$pid', 22, '$kodenya', '$periode', '$created_date', '$angka', '$posisi', '$nominal', '$menang', '$diskon', '$totalBayar', '$jumlahMenang', '', 0)") or die(mysqli_error());

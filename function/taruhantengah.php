...
if($saldoAktif < $subtotal){
	header('location:../taruhan.php?pid='.$pid.'&gameid=9&notif=1');
	exit();
}
else {
	for($i=1;$i<=10;$i++){
		...
		if($nominal != ''){
			$getGame = mysqli_query($conn,"SELECT * FROM `tb_game` WHERE parent = 9 AND title = '$angka'") or die(mysqli_error());
			...
			$insert_taruhan = mysqli_query($conn,"INSERT INTO `tb_taruhan` (`userID`, `pid`, `gameid`, `code`, `periode`, `created_date`, `tebak`, `posisi`, `nominal`, `menang`, `diskon`, `bayar`, `jumlah`, `keterangan`, `status`) VALUES ('$useridnya', '$pid', 9, '$kodenya', '$periode', '$created_date', '$angka', '', '$nominal', '$menang', '$diskon', '$totalBayar', '$jumlahMenang', '', 0)") or die(mysqli_error());
